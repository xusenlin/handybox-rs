...
- **轻量分发：** 当前本地构建产物的可执行文件约 **21–27 MiB**，ZIP 下载包约 **9–12 MiB**。
...
macOS 可执行文件大小不包含应用图标和包元数据。这些产物的构建日期不同，所含功能也可能不同，数据仅作为体积参考，不代表同一份代码的跨平台对比；重新构建当前源码后，体积可能变化。
...
拖放行为也会结合当前工具：图片工坊接收图片和文件夹，压缩包工具接收待打包文件夹，哈希与加密、文本对比可直接接收文件。在局域网共享页面开启共享后，拖入的文件会加入共享文件夹。
